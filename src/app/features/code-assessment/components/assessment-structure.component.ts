...
import { RunAnalysisOptions } from "../models/assessment.models";
    ...
    <div class="w-full max-w-7xl mx-auto px-4 py-8">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="bg-white rounded-lg border border-gray-200 shadow-sm">
            ...
            >
              <div>
                ...
                <p class="text-sm text-gray-600 mt-2">
                  Review the detected files and folders from your project
                </p>
              </div>
              <button
            ...
            <div class="p-6 max-h-[600px] overflow-y-auto">
              <div *ngIf="projectStructure" class="space-y-1">
                <div
                  class="flex items-center gap-2 py-2 px-3 bg-blue-50 rounded-lg font-semibold text-blue-900"
                >
                  <svg
        ...
        <div class="lg:col-span-1">
          <div
            class="bg-white rounded-lg border border-gray-200 shadow-sm sticky top-4"
          >
              ...
              >
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <span
                      *ngIf="!type.enabled"
                      class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium mt-2 bg-gray-200 text-gray-600"
                    >
                      Coming Soon
                    </span>
                  </div>
            ...
            <div class="border-t border-gray-200 p-6">
              <div class="flex flex-col gap-3">
                <button
                  (click)="onContinue()"
                  [disabled]="!hasSelection || isAnalyzing || !hasAnalyzableFiles"
                ...
                <p *ngIf="!hasAnalyzableFiles" class="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                  Assign at least one valid stage to one file before running analysis.
                </p>
...
export class AssessmentStructureComponent implements OnInit {
  ...
  @Output() runAnalysis = new EventEmitter<RunAnalysisOptions>();
  ...
  metricTypes: MetricType[] = [
    {
      ...
      name: "Cohesion",
      description:
        "Analyze code cohesion using module and package-level metrics",
      enabled: true,
      selected: false,
      config: {
  ...
  get hasAnalyzableFiles(): boolean {
    const mergedStages = this.getMergedStages();
    return Object.values(mergedStages).some((stages) => Array.isArray(stages) && stages.length > 0);
  }
  ...
  onContinue(): void {
    ...
    if (selectedType && this.hasAnalyzableFiles) {
      const payload: RunAnalysisOptions = {
        metrics: [...selectedType.config.metrics],
      };

      if (this.hasManualOverrides) {
        payload.pipeline_overrides = {
          file_stages: this.getManualOverrides(),
          excluded_files: [],
        };
      }
  ...
  private getMergedStages(): { [key: string]: string[] } {
    ...
    // 1. Populate with auto-detected stages (prefer file-centric map when available)
    if (this.autoDetectedPipeline?.file_stages) {
      Object.entries(this.autoDetectedPipeline.file_stages).forEach(
        ([file, stages]) => {
          const normalizedFile = this.normalizePath(file);
          if (!normalizedFile) {
            return;
          }
          merged[normalizedFile] = Array.isArray(stages)
            ? [...new Set(stages)]
            : [];
        },
      );
    } else if (this.autoDetectedPipeline?.detected_stages) {
      ...
      Object.entries(detectedStages).forEach(([stage, files]) => {
        if (files && Array.isArray(files)) {
          files.forEach((f: any) => {
            const filePath = this.normalizePath(String(f.file || ""));
            if (!filePath) {
              return;
            }
            if (!merged[filePath]) {
    ...
    Object.entries(this.manualFileStages).forEach(([file, stages]) => {
      ...
      // Keep empty arrays to explicitly remove auto-detected stages for a file.
      merged[normalizedFile] = [...stages];
    });
    return merged;
  }
  ...
  onFileStagesChange(event: { file: string; stages: string[] }): void {
    const normalizedFile = this.normalizePath(event.file);
    if (!normalizedFile) {
      return;
    }
    this.manualFileStages[normalizedFile] = event.stages;
  }

  private get hasManualOverrides(): boolean {
    return Object.keys(this.manualFileStages).length > 0;
  }

  private getManualOverrides(): { [key: string]: string[] } {
    ...
    Object.entries(this.manualFileStages).forEach(([file, stages]) => {
      ...
      overrides[normalizedFile] = Array.isArray(stages) ? [...new Set(stages)] : [];
    });
  ...
  template: `
    <div>
      ...
      >
        ...
        <span
          *ngIf="!isDirectory && node.size !== undefined"
        ...
        <span
          *ngIf="!isDirectory && node.valid_syntax === false"
        ...
        <div
          *ngIf="!isDirectory && !isEditing && getDetectedStages().length > 0"
          class="flex gap-1 flex-shrink-0"
        >
          <span
            *ngFor="let stage of getDetectedStages()"
        ...
        <div
          *ngIf="!isDirectory && isEditing"
          class="flex gap-1 flex-shrink-0 ml-auto"
        ...
        >
          <div class="relative">
            ...
            <div
              *ngIf="isStageSelectorOpen"
              class="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 w-64 p-2"
            >
              <div class="flex justify-between items-center mb-2 px-2">
                <span class="text-xs font-semibold text-gray-500"
                  >Select Stages (Max 5)</span
                >
                <button
                  (click)="toggleStageSelector()"
          ...
          <div class="flex gap-1 ml-2">
            <span
              *ngFor="let stage of getEffectiveStages()"
              class="px-2 py-0.5 text-xs font-medium rounded"
              [ngClass]="getStageColor(stage)"
            >
              {{ formatStageName(stage) }}
...
export class TreeNodeComponent {
  ...
  @Output() stagesChange = new EventEmitter<{
    file: string;
    stages: string[];
  }>();
  ...
  private getStagesForNode(node: ChildChild): string[] {
    const normalizedNodePath = this.normalizePath(node.path || node.name || "");

    // Check manual stages
    for (const [path, stages] of Object.entries(this.manualStages)) {
      if (this.pathsMatch(normalizedNodePath, this.normalizePath(path))) {
        return stages;
      }
    }

    return this.getAutoDetectedStagesForNode(normalizedNodePath);
  }
  ...
  getDetectedStages(): string[] {
    ...
    const normalizedNodePath = this.normalizePath(
      this.node.path || this.node.name || "",
    );
    return this.getAutoDetectedStagesForNode(normalizedNodePath);
  }

  private getAutoDetectedStagesForNode(normalizedNodePath: string): string[] {
    if (!this.detectedPipeline) {
      return [];
    }

    // Prefer file-centric representation from backend.
    if (this.detectedPipeline.file_stages) {
      const exact = this.detectedPipeline.file_stages[normalizedNodePath];
      if (exact) {
        return [...exact];
      }

      for (const [filePath, stages] of Object.entries(
        this.detectedPipeline.file_stages,
      )) {
        if (this.pathsMatch(normalizedNodePath, this.normalizePath(filePath))) {
          return [...stages];
        }
      }
    }
      ...
      if (files && Array.isArray(files)) {
        if (
          files.some((f: any) =>
            this.pathsMatch(
              normalizedNodePath,
              this.normalizePath(String(f.file || "")),
            ),
          )
        ) {
          stages.push(stage);
  ...
  getEffectiveStages(): string[] {
    ...
    const hasManual = Object.keys(this.manualStages).some((path) => {
      return this.pathsMatch(normalizedNodePath, this.normalizePath(path));
    });
